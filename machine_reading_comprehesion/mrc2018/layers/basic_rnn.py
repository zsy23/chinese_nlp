...
def get_cell(rnn_type, hidden_size, layer_num=1, dropout_keep_prob=None):
    ...
    """
    if rnn_type.endswith('lstm'):
        cell = tc.rnn.LSTMCell(num_units=hidden_size, state_is_tuple=True, initializer=tf.orthogonal_initializer(), forget_bias=0.0)
    elif rnn_type.endswith('gru'):
        cell = tc.rnn.GRUCell(num_units=hidden_size, kernel_initializer=tf.orthogonal_initializer())
