...
传统的序列学习都是使用RNN的，作者提出完全建立在CNN上的新架构。CNN能够实现完全的并行，并且非线性操作已经固定，和输入序列长度也无关，使得优化更加容易。作者还提出使用线性门单元，使得梯度传播更加容易，还为每个解码层都配备了独立的注意力模型。

首先需要将输入$\bold x=(x_1,...,x_m)$嵌入到分布空间$\bold w=(w_1,...,w_m)$中，其中$w_j\in R^f$是词向量矩阵$D\in R^{V\times f}$的一列。为了使得模型能够感知到序列顺序，把输入的绝对位置信息也进行嵌入$\bold p=(p_1,...,p_m)$，其中$p_j\in R^f$。然后两种嵌入结合起来得到最终输入的表示$\bold e=(w_1+p_1,...,w_m+p_m)$。对解码网络已经生成的输出进行同样操作得到输出表示$\bold g=(g_1,...,g_n)$，从而能够继续输入到解码网络中。

编码器解码器共享一套简单的块结构，用来计算固定数目输入的中间状态。把第$l$个块的输出表示为$\bold h^l=(h_1^l,...,h_n^l)$（解码器）和$\bold z^l=(z_1^l,...,z_n^l)$（编码器）。每一个块包含一个一维的卷积，然后再是一个非线性函数。对于只有一个块的解码网络，核宽度为$k$。每一个输出$h_i^1$包含$k$个输入的信息。把很多块堆叠起来，增加了包含在一个状态中的输入信息个数。非线性函数使得网络探索到整个输入空间。每一个卷积核的参数为$W\in R^{2d\times kd},b_w\in R^{2d}$，输入为$X\in R^{k\times d}$，是$k$个$d$维输入的连接，映射到输出$Y\in R^{2d}$上，维度是输入的两倍，接下来层的操作是以上一层$k$个输出为输入的。选用了线性门单元(GLU)作为非线性函数，在卷积的输出$Y=[A\ B]\in R^{2d}$上实现了简单的门机制：

$$
v([A\ B])=A\otimes \sigma(B)
$$ (1)

其中$A,B\in R^d$，$\otimes$是逐元素乘积，输出$v([A B])\in R^d$是$Y$大小的一半。门$\sigma(B)$控制$A$中多少和输出是有关的。为了实现深层的卷积，对于每个块的输出采用了残差连接，

$$
h_i^l=v(W^l[h_{i-k/2}^l-1,...,h_{i+k/2}^l-1]+b_w^l)+h_i^{l-1}
$$ (2)

对于编码网络，通过对卷积的输出进行padding，保证满足输入到下一层的长度要求。但是对于解码网络，任何未来的信息不应该被考虑进来，因此在输入的左右两边都pad $k-1$个零向量，然后把输出的末尾$k$个元素去掉。

加上线性变换操作，使得词向量的$f$维和卷积的输出$2d$维能够映射起来。把这种变换$\bold w$加到词向量输入编码器前，加到编码器输出上和解码器输出在进入softmax前，及其解码器在计算注意力前。

最后计算可能的$T$个下一个目标元素$y_{i+1}$的分布：

$$
p(y_{i+1}|y_1,...,y_i,\bold x)=softmax(W_oh_i^L+b_o)\in R^T
$$ (3)

对于每一层解码网络，加上独立的注意力模型。为了计算注意力，把目前的解码状态$h_i^l$和前一个目标元素$g$的词向量结合起来：

$$
d_i^l=W_d^lh_i^l+b_d^l+g_i
$$ (4)

对于第$l$层解码网络，状态$i$和源词$j$的注意力$\alpha_{ij}^l$是解码器的状态总结$d_i^l$和编码器第$u$个块的输出$z_j^u$的点积：

$$
\alpha_{ij}^l=\frac{\exp(d_i^l\cdot z_j^u)}{\sum_{t=1}^m\exp(d_i^l\cdot z_t^u)}
$$ (5)

解码层的条件输入$c_i^l$是编码器输出和输入词向量的和的加权和：

$$
c_i^l=\sum_{j=1}^m a_{ij}^l(z_j^u+e_j)
$$ (6)

这和RNN中只用$z_j^u$计算注意力和加权和是不一样的。作者发现加上$e_j$是有用的，并且很像是键值记忆网络，其中键是$z_j^u$，值是$z_j^u+e_j$。编码器的输出$z_j^u$包含了大量输入的上下文信息，$e_j$提供了每个输入元素的信息，这些对于推断是有用的。一旦$c_i^l$计算出来，他被加到解码层的输出$h_i^l$上。这可以被看作是multi-hops的注意力机制，实际上，第一层的注意力决定了哪些输入上下文是有用的，然后输入到下一层中。解码器能够直接获取前$k-1$个时刻的注意力历史信息，因为$h_i^l$的输入$h_{i-k}^{l-1},...,h_i^{l-1}$包含了上下文输入$c_{i-k}^{l-1},...,c_i^{l-1}$的信息。这使得模型能够更加容易考虑到已经被注意的之前的输入，但在RNN中这些信息却要通过好几个非线性处理。

卷积结构使得可以batch处理所有元素的注意力机制，作者把每个解码层的注意力分开处理。总体架构如下图所示：

![conv_seq2seq](conv_seq2seq.png)

下面介绍归一化策略。为了使学习过程更加稳定，需要小心地进行权重初始化和网络部分的伸缩处理，使得网络的方差不至于变化地太夸张。实际上，把残差块和注意力的输出都要进行伸缩，使得激活后的方差保持不变。把残差块的输入和输出的和要乘以$\sqrt{0.5}$，来使得和的方差减小一半。这意味着每一项的方差相同，这虽然不怎么现实，但是实际效果很好。注意力生成的条件输入$c_i^l$是$m$个向量的加权和，为了应对方差的变化，需要乘以$m\sqrt{1/m}$，乘以$m$是为了使输入保持原始大小，这里假设每一个注意力得分是均衡分布的。这也不怎么现实，但是效果不错。对于多层注意力的卷积解码器，把输入层的梯度乘以使用的注意力的数目，排除了源词的词向量。

下面介绍初始化处理。初始化处理也是为了使得网络的前向和后向传播方差变化不会太大。所有的词向量通过均值0，标准差0.1的正态分布进行初始化。假设输入维度为$n_l$，keep_dropout为$p$，对于输出要输入到GLU的层，初始化使用$\mathcal N(0,\sqrt{4p/n_l})$，其他层使用$\mathcal N(0,\sqrt{p/n_l})$，具体推导查看原文附录。
