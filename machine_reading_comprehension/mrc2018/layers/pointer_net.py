...
def summ(memory, hidden, scope="summ"):
    with tf.variable_scope(scope):
        s0 = tc.layers.fully_connected(memory, num_outputs=hidden, activation_fn=tf.nn.tanh)
        s = tc.layers.fully_connected(s0, num_outputs=1, activation_fn=None)
        s1 = tf.squeeze(s, [2])
        a = tf.expand_dims(tf.nn.softmax(s1), axis=2)
        res = tf.reduce_sum(a * memory, axis=1)
        return res

def pointer(inputs, state, hidden, scope="pointer"):
    with tf.variable_scope(scope):
        u = tf.concat([tf.tile(tf.expand_dims(state, axis=1), [1, tf.shape(inputs)[1], 1]), inputs], axis=2)
        s0 = tc.layers.fully_connected(u, num_outputs=hidden, activation_fn=tf.nn.tanh)
        s = tc.layers.fully_connected(s0, num_outputs=1, activation_fn=None)
        s1 = tf.squeeze(s, [2])
        a = tf.expand_dims(tf.nn.softmax(s1), axis=2)
